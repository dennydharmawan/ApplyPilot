...
description: Use when tailoring a resume for a job application. Research the role, match evidence from the resume bank, preserve factual integrity, and generate reviewable resume files.
---
...
Use these defaults unless the user provides a different source or output location.

- Resume bank: `$APPLYPILOT_DIR/resumes/`
- Denny resume bank in this checkout: `/Users/denny.dharmawan/ApplyPilot/profiles/denny/resumes/`
- Golden resume: `/Users/denny.dharmawan/portfolios/personal-site/public/resume.pdf`
- Application outputs: `$APPLYPILOT_DIR/resumes/applications/<company-slug>-<role-slug>/`

Read the golden resume first. Treat it as the baseline for identity, contact details, career facts, section hierarchy, length, and visual standard. Use the resume bank for additional truthful evidence and approved variants. Surface conflicts instead of silently choosing one source.

The resume bank may be empty. In that case, build the initial library from the golden resume and continue. Scan the bank for Markdown, PDF, and DOCX resumes. Skip generated reports and unrelated files.

Create every tailored resume as a separate file. Never overwrite the golden resume. Preserve its presentation unless the target role or the user requires a specific change.

Write drafts, reports, and final files to the job-specific application output directory. Because that directory is already inside the resume bank, library approval keeps the files in place and refreshes the in-memory library.
...
2. Optional resume bank override. The default is `$APPLYPILOT_DIR/resumes/`.

**Workflow:**
1. Build the library from the golden resume and resume bank
...
6. Generate MD + DOCX + Report, plus PDF when requested
...
1. **Locate resume sources:**
   ```
   Golden resume: /Users/denny.dharmawan/portfolios/personal-site/public/resume.pdf
   Resume bank: user override OR $APPLYPILOT_DIR/resumes/
   Validate both paths when present
   ```

2. **Scan the resume bank:**
   ```
   Scan for *.md, *.pdf, and *.docx files
   Skip generated reports and unrelated files
   Count files found
   Announce: "Building resume library... found {N} bank resumes plus the golden resume"
   ```

3. **Parse each resume:**
   Parse the golden resume first, then each resume-bank file:
   - Read the file content
   - Use the available PDF or document skill for binary formats
...
```
Search the web with these queries:
...
```
Search for: "site:linkedin.com {job_title} {company}"
Read the top 3-5 relevant profiles
...
**Use the available DOCX document skill or tool:**
...
```
Use the available PDF skill or tool.
...
**Edge Case 1: Golden Resume Unavailable and Resume Bank Limited**
```
SCENARIO: Golden resume is missing or unreadable and the resume bank has only 1-2 resumes
...
An empty resume bank is not an error while the golden resume is readable. Build the library from the golden resume and continue without this checkpoint.
...
```
SCENARIO: Web research fails, LinkedIn is unavailable, or company information is sparse
